...
## Phase 1: Scaffold & Strip Cloud Dependencies
- [ ] Remove Manus-specific vite plugin and platform config
- [ ] Update package.json: remove @aws-sdk, add bcrypt, chokidar, node-cron
- [ ] Replace server/_core/env.ts with local env vars
- [ ] Replace server/storage.ts with local filesystem storage
- [ ] Remove/stub Manus-only _core modules (heartbeat, notification, dataApi, map, storageProxy)
- [ ] Update drizzle.config.ts to use SQLite by default (with MySQL option for Docker)

## Phase 2: Local Authentication
- [ ] Replace Manus OAuth with local username/password auth (bcrypt + JWT)
- [ ] Add users table with username/password/role to drizzle schema
- [ ] Add login page and first-run admin setup page
- [ ] Update protectedProcedure and adminProcedure to use local auth
- [ ] Update client useAuth hook for local auth
- [ ] First-run wizard: if no admin exists, redirect to /setup

## Phase 3: Local Filesystem Scanner
- [ ] Replace driveScanner.ts with localScanner.ts (recursive fs.readdir)
- [ ] Support configuring library root path via Settings (stored in DB)
- [ ] Remove driveImageProxy.ts — serve images directly from disk via Express static
- [ ] Update scan routers to call localScanner
- [ ] Update Settings page: replace API Key / Folder ID with Library Folder Path
- [ ] Replace Drive-based parent detection with fs-based parent detection in rescanOne

## Phase 4: Local File Storage
- [ ] Replace storagePut/storageGet with local filesystem read/write
- [ ] Serve /local-files/* via Express static middleware pointing at library folder
- [ ] Update ZIP download to read from local disk
- [ ] Update all /manus-storage/* URL references to /local-files/*

## Phase 5: Configurable LLM
- [ ] Replace server/_core/llm.ts with OpenAI-compatible client (reads LLM_API_URL + LLM_API_KEY + LLM_MODEL from env/DB)
- [ ] Add LLM configuration section to Settings page
- [ ] Gracefully disable AI tagging and thumbnail picker if no LLM configured
- [ ] Show "AI features disabled — configure LLM in Settings" notice when unconfigured

## Phase 6: Branding & First-Run Wizard
- [ ] Add branding settings to DB: app_name, tagline, logo_path
- [ ] Add Branding section to Settings page (app name, tagline, logo upload)
- [ ] Replace all hardcoded "Kenny Print It?" with dynamic app_name
- [ ] Replace logo with dynamic branding logo
- [ ] First-run wizard: step 1 = branding, step 2 = admin password, step 3 = library folder path

## Phase 7: Docker & Windows Packaging
- [ ] Write Dockerfile (Node 22 Alpine)
- [ ] Write docker-compose.yml (app + MySQL, volume mounts for library folder + data)
- [ ] Write .env.example with all env vars documented
- [ ] Write Start.bat and Stop.bat for Windows
- [ ] Write README-INSTALL.md with step-by-step Windows install guide
- [ ] Add /api/health endpoint for Docker health check

## Phase 8: Testing & Delivery
- [ ] Install dependencies and confirm build succeeds
- [ ] Run all tests and fix any broken ones
- [ ] Smoke test: scan local folder, browse models, login/logout, change branding
- [ ] Deliver Local version v1.0

## Round 18: Multi-path Libraries, Open in Slicer, Show in Explorer

- [ ] Schema: add library_paths table (id, path, label, enabled, sortOrder)
- [ ] Schema: add rootPath column to models table to track which library root each model came from
- [ ] localScanner.ts: accept array of root paths, scan all of them, tag each model with its rootPath
- [ ] routers.ts: settings.getLibraryPaths, settings.addLibraryPath, settings.removeLibraryPath, settings.toggleLibraryPath procedures
- [ ] routers.ts: scan.start uses all enabled library paths
- [ ] Settings UI: replace single path input with multi-path manager (add/remove/toggle/label paths)
- [ ] ModelDetail: show "Offline" badge if rootPath drive is not currently mounted/accessible
- [ ] ModelDetail: "Open in Slicer" button using my3dlibrary:// custom URL protocol
- [ ] ModelDetail: "Show in Explorer" button using my3dlibrary:// protocol to open folder in Windows Explorer
- [ ] Protocol handler: write protocol-handler/index.js (Node.js helper that handles open and explore commands)
- [ ] Protocol handler: package as standalone exe using pkg
- [ ] NSIS installer: register my3dlibrary:// protocol in Windows registry during install
- [ ] NSIS installer: unregister protocol on uninstall
- [ ] Rebuild Standard and Pro installers with all new features
